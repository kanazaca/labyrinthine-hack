...
namespace Labyrinthine.Utilities
{
    internal class Drawing
    {
        ...
        public static void TextWithDistance(Transform target, string text, Camera relativeTo = null)
        {
            ...
            {
                float distanceToMonster = (float)Math.Round((double)Vector3.Distance(Main.PlayerControl.transform.position, target.position), 1);

                if (vector.z >= 0f && distanceToMonster < 125f)
                {
                    DrawString(new Vector2(vector.x, (float)Screen.height - vector.y), text + " [" + distanceToMonster.ToString() + "m]", Color.cyan, 12, true);
                }
            }
        }
    }
